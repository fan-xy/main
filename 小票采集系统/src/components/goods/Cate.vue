<template>
  <div>
    ...
    <el-card>
      ...
      <!-- 第三方插件 树形表格 show-row-hover:鼠标悬停是否高亮  show-index index-text：显示索引 索引名 colums列名-->
...
export default {
  data() {
    return {
      ...
      // 为table指定列的定义
      columns: [
        //每个对象表示一列，lable表示列名，prop这一列所绑定到的数据属性
        {
          label: '分类名称',
          prop: 'cat_name' 
        },
        {
          ...
          // 表示，将当前列定义为模板列
          type: 'template',
          // 表示当前这一列使用模板名称 在表格中用<template slot='isok'>定义slot-scope属性接收这一行数据
          template: 'isok'
        },
        {
          label: '排序',
      ...
      // 指定级联选择器的配置对象
      cascaderProps: {
        value: 'cat_id',   //该属性指定具体选择的属性
        label: 'cat_name', //该属性指定展示的属性
        children: 'children' //该属性指定级联
      },
      // 选中的父级分类的Id数组
      selectedKeys: []
    }
